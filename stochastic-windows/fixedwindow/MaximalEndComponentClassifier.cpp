...
namespace sw {
    namespace FixedWindow {
        ...
        MaximalEndComponentClassifier<ValueType>::MaximalEndComponentClassifier(
            ...
            if (produceScheduler) {
                sw::DirectFixedWindow::WindowMemory<ValueType> windowMemory = windowUnfolding.generateMemory(memoryStatesLabeling);
                ...
                for (uint_fast64_t const& k : this->goodMECs) {
                    storm::utility::graph::computeSchedulerProb1E(*mecStates[k], // good states have a probability one of reaching safe states belonging to the same MEC
        ...
        MaximalEndComponentClassifier<ValueType>::MaximalEndComponentClassifier(
            ...
            for (uint_fast64_t k = 0; k < mecDecompositionGame.size(); ++ k) {
                ...
                gameResult = produceScheduler ? mecGame.produceSchedulerForDirectFW(memoryStatesLabeling)
            ...
            if (produceScheduler) {
                ...
                // state labels
                storm::models::sparse::StateLabeling stateLabeling = memoryStatesLabeling and not this->goodStateSpace.empty() ?
                                                                     schedulersVector[this->goodStateSpace.getNextSetIndex(0)]->getMemoryStructure()->getStateLabeling()
                                                                     : storm::models::sparse::StateLabeling(mecDecompositionGame.getMaximumWindowSize());
                // Initial memory states
                std::vector<uint_fast64_t> initialMemoryStates(mdp.getInitialStates().getNumberOfSetBits(), 0);
                storm::storage::MemoryStructure memoryStructure(std::move(memoryUpdates), std::move(stateLabeling), std::move(initialMemoryStates));
                this->mecScheduler = std::unique_ptr<storm::storage::Scheduler<ValueType>>(
