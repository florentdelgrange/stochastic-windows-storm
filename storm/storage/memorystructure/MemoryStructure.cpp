...
namespace storm {
    namespace storage {
        ...
        template<typename ValueType, typename RewardModelType>
        uint_fast64_t MemoryStructure::getSuccessorMemoryState(models::sparse::Model<ValueType, RewardModelType> const& model,
                                                               uint_fast64_t const& currentMemoryState,
                                                               uint_fast64_t const& currentModelState, uint_fast64_t const& choice,
                                                               uint_fast64_t const& arrivalModelState) const {
            uint_fast64_t row = model.getTransitionMatrix().getRowGroupIndices()[currentModelState] + choice;
            auto entryIt = model.getTransitionMatrix().getRow(row).begin();
            while (entryIt->getColumn() != arrivalModelState) {
                ++ entryIt;
            }
            uint_fast64_t transitionIndex = entryIt - model.getTransitionMatrix().begin();
            return this->getSuccessorMemoryState(currentMemoryState, transitionIndex);
        }
        ...
        template uint_fast64_t MemoryStructure::getSuccessorMemoryState(const storm::models::sparse::Model<double> &model, uint_fast64_t const &currentMemoryState, uint_fast64_t const &currentModelState, uint_fast64_t const &choice, uint_fast64_t const &arrivalModelState) const;
        ...
        template uint_fast64_t MemoryStructure::getSuccessorMemoryState(const storm::models::sparse::Model<storm::RationalNumber> &model, uint_fast64_t const &currentMemoryState, uint_fast64_t const &currentModelState, uint_fast64_t const &choice, uint_fast64_t const &arrivalModelState) const;
