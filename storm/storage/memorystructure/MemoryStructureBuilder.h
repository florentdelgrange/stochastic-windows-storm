...
namespace storm {
    namespace storage {
        ...
        public:
            ...
            /*!
             * Specifies a transition of the memory structure.
             * The interpretation is that we switch from startState to goalState upon entering one of the specified model states via the specified choice.
             *
             * @note If it is not possible to move from startState to goalState, such a transition does not have to be set explicitly.
             * @note fill in the current transitions matrix
             *
             * @param startState the memorystate in which the transition starts
             * @param goalState the memorystate in which the transition ends
             * @param modelStates model states that triggers this transition.
             * @param modelChoice filers the choice of the model that trigger this transition.
             *
             */
            void setTransition(uint_fast64_t const& startState, uint_fast64_t const& goalState, storm::storage::BitVector const& modelStates, uint_fast64_t const& modelChoice);
